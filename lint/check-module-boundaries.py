...
for path in list(SRC.rglob("*.hpp")) + list(SRC.rglob("*.cpp")):
    ...
    owner = rel.parts[0] if len(rel.parts) > 1 else ""
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8", errors="ignore").splitlines(), 1):
        ...
        if target in MODULES and target != owner and target not in MODULES[owner]:
            violations.append(f"{path.relative_to(ROOT)}:{line_no}: {owner} may not include {target}")

if checked == 0:
    print("module-boundary check failed: 0 files checked", file=sys.stderr)
    sys.exit(1)
